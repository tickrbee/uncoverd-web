...
export async function GET(): Promise<NextResponse> {
  ...
  const staticUrls: { path: string; freq: Entry["changefreq"]; pri: number }[] = [
    ...
    { path: "/tools/portfolio-healthcheck", freq: "monthly", pri: 0.8 },
    { path: "/tools/portfolio-generator", freq: "monthly", pri: 0.8 },
    { path: "/dividend-increases", freq: "daily", pri: 0.85 },
  ];
  ...
  // Localized dividend-grower landing pages (aristocrats/kings/…).
  for (const loc of CONTENT_LOCALES) {
    for (const g of GROWER_TAXO) {
      entries.push({
        loc: `${BASE}/${loc}/${GROWER_PATH[loc]}/${g.slug[loc]}`,
        lastmod: now,
        changefreq: "weekly",
        priority: 0.65,
      });
  ...
  // Localized industry landing pages (REITs/MLPs/semiconductors/…).
  for (const loc of CONTENT_LOCALES) {
    for (const i of INDUSTRY_TAXO) {
      entries.push({
        loc: `${BASE}/${loc}/${INDUSTRY_PATH[loc]}/${i.slug[loc]}`,
        lastmod: now,
        changefreq: "weekly",
        priority: 0.6,
      });
  ...
  // Localized payout-changes pages (increasing/decreasing/…).
  for (const loc of CONTENT_LOCALES) {
    for (const p of PAYOUT_TAXO) {
      entries.push({
