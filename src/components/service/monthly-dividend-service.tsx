...
// Reusable localized "monthly dividend stocks" service page. Targets the
// monthly keywords (action à dividende mensuel, monatliche dividende aktien,
// acciones dividendo mensual, azioni dividendo mensile) using the global set of
// monthly-paying stocks. Reads no cookies → ISR-cacheable.
export async function MonthlyDividendService({
  locale,
  strings,
  cover,
  coverAlt,
  ...
  let rows: Awaited<ReturnType<typeof listMonthlyPayers>> = [];
  try {
    rows = await listMonthlyPayers(40);
  } catch {
    rows = [];
  }
                  ...
                  <tbody>
                    {rows.map((r) => (
                      <tr key={r.symbol}>
                        ...
                        <td className="dv-td--num">{formatCurrency(r.price, { currency: r.currency })}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
